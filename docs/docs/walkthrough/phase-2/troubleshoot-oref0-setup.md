...
This will open a text editor where you can add your raw pump settings.  You can use the examples below for each of the files required.

**THESE ARE PERSONAL SETTINGS!  ENSURE YOU MAKE CHANGES TO THE SETTINGS TO MATCH WHAT IS CURRENTLY ON YOUR PUMP**

Once your personalizations are complete, paste the code into the text editor, and type Ctl-X.  When it asks if you want to save, select Yes, and keep the settings.json name

Repeat the steps above for the following files: bg-targets-raw.json, and the selected-basal-profile.json.  Again, you will see samples below for each of these files.  THEY MUST ALSO BE PERSONALIZED!
